...
def lambda_handler(event, context):
    ...
    if is_backfill:
        to_date = (
            date.fromisoformat(event["to_date"])
            if event.get("to_date")
            else date.today()
        )
        from_date = (
            date.fromisoformat(event["from_date"])
    ...
    try:
        for ticker in tickers:
            ...
            if len(raw_articles) > MAX_ARTICLES_PER_TICKER_PER_DAY:
                logger.info(
                    "Capping %s to %d articles", ticker, MAX_ARTICLES_PER_TICKER_PER_DAY
                )
            ...
            if len(raw_articles) > remaining_budget:
                logger.info(
                    "Capping %s to %d articles (daily budget of %d reached)",
                    ticker,
                    remaining_budget,
                    MAX_ARTICLES_PER_DAY_TOTAL,
                )
            ...
            rows = [
                to_article_row(article, fetch_article_text(article["url"]))
