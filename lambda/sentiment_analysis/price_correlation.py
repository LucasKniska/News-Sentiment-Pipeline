...
def _report_threshold_sweep(rows: list[dict]) -> None:
    ...
    for t in _THRESHOLDS:
        kept = [
            (r["sentiment"] >= t, r["actual_up"])
            for r in rows
            if abs(r["sentiment"]) >= t
        ]
        if not kept:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Correlate AI-extracted sentiment direction against actual daily price direction."
    )
    parser.add_argument(
        "--horizon",
        type=int,
    ...
    if not rows:
        raise SystemExit(
            "No overlapping sentiment/price rows found - run lambda/price_backfill and lambda/sentiment_analysis/run.py first."
        )

    print(
        f"Loaded {len(rows)} ticker/day observations (involvement floor {_INVOLVEMENT_FLOOR}, horizon={args.horizon})."
    )
