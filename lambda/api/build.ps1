# Packages lambda/api into build/ for deployment: installs Linux/cp313
# wheels for the one compiled dep (psycopg-binary) directly on Windows via
# uv's cross-platform resolution, then flattens the handler source files in
# alongside them. Does NOT zip - terraform's archive_file data source owns
# that, so source_code_hash tracks changes. Re-run this before every
# `terraform plan`/`apply` that should pick up a code or dependency change.
#
...
# package's size for a dependency the runtime already provides.
$runtimeProvidedPackages = "boto3", "botocore", "jmespath", "s3transfer"
$filteredRequirementsPath = Join-Path $env:TEMP "news_sentiment_api_lambda_requirements.txt"
